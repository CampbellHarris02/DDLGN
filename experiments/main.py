...
def eval(model, loader, mode):
    ...
    with torch.no_grad():
        ...
        res = np.mean(
            [
                (model(x.to('mps').round()).argmax(-1) == y.to('mps')).to(torch.float32).mean().item()
                for x, y in loader
            ]
        )
...
def packbits_eval(model, loader):
    ...
    with torch.no_grad():
        ...
        res = np.mean(
            [
                (model(PackBitsTensor(x.to('mps').reshape(x.shape[0], -1).round().bool())).argmax(-1) == y.to(
                    'mps')).to(torch.float32).mean().item()
                for x, y in loader
            ]
        )
        model.train(mode=orig_mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--implementation', type=str, default='mps', choices=['mps', 'python'],
                        help='`mps` is the fast METAL implementation and `python` is simpler but much slower '
    ...
    for i, (x, y) in tqdm(
        ...
        x = x.to(BITS_TO_TORCH_FLOATING_POINT_TYPE[args.training_bit_count]).to('mps')
        y = y.to('mps')
